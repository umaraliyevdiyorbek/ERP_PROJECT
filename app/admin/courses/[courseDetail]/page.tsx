...
const SingleCourse = () => {
  ...
  useEffect(() => {
    const getCourse = async () => {
      try {
        const response = await $http.get(`/course/${params.courseDetail}`)
        setCourse({
          ...response.data,
  ...
  return (
    <div className='flex justify-center items-center h-screen'>
      ...
      <div  className="position-absolute top-50 start-50 translate-middle ml-62 m-40 bg-indigo-400 block  max-w-sm p-6 border text-center border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
        <h5 className="mb-3 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">Course Id: {course?.course_id}</h5>
        <hr />
        <h5 className="mb-3 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">Course Name: {course?.course_name}</h5>
        <hr />
        <p className="font-normal mb-3 text-gray-700 dark:text-gray-400">Course create at: {course ? format(course.createdAt, 'dd-MM-yyyy') : ''}</p>
        <hr />
        <p className="font-normal mb-3 text-gray-700 dark:text-gray-400">Course update at: {course ? format(course.updatedAt, 'dd-MM-yyyy') : ''}</p>
      </div>
