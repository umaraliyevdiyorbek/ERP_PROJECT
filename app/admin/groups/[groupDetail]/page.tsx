...
const SingleGroup = () => {
  ...
  useEffect(() => {
    const getGroup = async () => {
      try {
        const response = await $http.get(`/group/${params.groupDetail}`)
        setGroup({
          ...response.data,
  ...
  return (
    <div className="flex justify-center items-center h-screen">
      <div className="position-absolute top-50 start-50 translate-middle ml-62 m-40 bg-indigo-400 block  max-w-sm p-6 border text-center border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
        <h5 className="mb-3 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">Group Id: {group?.group_id}</h5>
        <hr />
        <h5 className="mb-3 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">Group Name: {group?.group_name}</h5>
        <hr />
        <p className="font-normal mb-3 text-gray-700 dark:text-gray-400">Group create at: {group ? format(group?.createdAt, 'dd-MM-yyyy') : ''}</p>
        <hr />
        <p className="font-normal mb-3 text-gray-700 dark:text-gray-400">Group update at: {group ? format(group?.updatedAt, 'dd-MM-yyyy') : ''}</p>
      </div>
