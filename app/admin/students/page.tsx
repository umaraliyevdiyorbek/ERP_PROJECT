...
const Students: React.FC = () => {
  ...
  const handleEdit = async (id: number) => {
    const editValue = prompt('Update student name')
    if (!editValue) return
    try {
      const { data } = await $http.put(`/student/${id}`, { student_name: editValue }, { headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${Cookies.get('token')}` } })
      setStudents((prevStudents) => prevStudents.map((student) => (student.id === id ? { ...student, student_name: data.student_name } : student)))
    } catch (error) {
  ...
  const handleView = async (id: number) => {
    try {
      const { data } = await $http.get(`/student/${id}`, { headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${localStorage.getItem('token')}` } })
  ...
  const handleCreateStudent = async () => {
    try {
      const { data } = await $http.post('/student/create', { student_name: studentName, username: username, password: password, group_id: ID }, { headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${Cookies.get('token')}` } })
  ...
  const handleDelete = async (id: number) => {
    ...
    if (check) {
      try {
        const { data } = await $http.delete(`/student/${id}`, { headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${Cookies.get('token')}` } })
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-8 d-flex">
      <div className="relative overflow-x-auto shadow-md sm:rounded-lg ml-80">
        <button className="mb-4 px-4 py-2 bg-indigo-500 hover:bg-indigo-700 text-black rounded-lg" onClick={createStudent}>
          Add Student
        </button>
          ...
          <tbody>
            {students?.map((student) => (
              <tr key={student.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                ...
                <td className="px-6 py-4">{student.group_id}</td>
                <td className="px-6 py-4">
                  <div className="flex space-x-2">
                    <button className="px-4 py-2 bg-blue-500 text-white rounded-lg" onClick={() => handleEdit(student.id)}>
                      Edit
                    </button>
                    <button className="px-4 py-2 bg-green-500 text-white rounded-lg" onClick={() => handleView(student.id)}>
                      View
                    </button>
                    <button className="px-4 py-2 bg-red-500 text-white rounded-lg" onClick={() => handleDelete(student.id)}>
                      Delete
                    </button>
                  </div>
                </td>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-10 overflow-y-auto flex justify-center items-center">
          ...
          <div className="relative bg-white p-8 rounded-lg max-w-md">
            ...
            <form id="submit" onSubmit={handleCreateStudent}>
              <input onChange={(e) => setStudentName(e.target.value)} type="text" placeholder="Enter your first name" name="student" className="border border-gray-300 rounded-md px-4 py-2 w-full mb-4" />
              <input onChange={(e) => setUserName(e.target.value)} type="text" placeholder="Enter your username" name="student" className="border border-gray-300 rounded-md px-4 py-2 w-full mb-4" />
              <input onChange={(e) => setPassword(e.target.value)} type="password" placeholder="Enter your password" name="student" className="border border-gray-300 rounded-md px-4 py-2 w-full mb-4" />
              <input onChange={(e) => setID(e.target.value)} type="text" placeholder="Enter your group ID" name="student" className="border border-gray-300 rounded-md px-4 py-2 w-full mb-4" />
              <div className="flex justify-end">
                <button className="px-4 py-2 bg-blue-500 text-white rounded-lg">Add Student</button>
